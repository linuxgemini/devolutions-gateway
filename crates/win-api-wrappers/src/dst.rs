...
impl<Def: Win32DstDef> Win32Dst<Def> {
    ...
    pub fn push(&mut self, value: Def::Item) {
        ...
        unsafe { self.inner.realloc(new_size).expect("OOM") };
        ...
        let array_ptr = unsafe { self.inner.as_mut_ptr().byte_add(Def::ARRAY_OFFSET) };
        ...
        // Update the number of privileges.

        // SAFETY: Per invariants, the offset is in bounds of the container.
        let count_ptr = unsafe { self.inner.as_mut_ptr().byte_add(Def::ITEM_COUNT_OFFSET) };
