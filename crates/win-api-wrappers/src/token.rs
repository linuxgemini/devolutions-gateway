...
impl Token {
        ...
        anyhow::Error: for<'a> From<<D as TryFrom<&'a T>>::Error>,
    {
        ...
        // SAFETY: Per safety preconditions, the target type is properly matching the struct initialized for the given info class.
        let info = unsafe { info.as_ref_cast::<T>() };

        let info = D::try_from(info)?;
